# Using Shelf Life

The site is one long page; the room links (a rail on wide screens, a dock at the bottom of a phone) jump between rooms. The figures at the top of the dock are your stay so far. Tap them to reach **Time**.

**Look.** The arrival. _Look around_ shows the gallery from different angles.

**Read.** _Pull a book_ takes a random page from anywhere in the Library and shows exactly where it is shelved. The hexagon number is thousands of digits long; _Show all digits_ unfolds it. _Turn the page_ moves to the next page of the same book, which is as unrelated to the last as any other page. About one pull in eight shows a page with a line that almost makes sense. It is a real page, at the address shown.

**Search.** _Check a shelf_ checks a few thousand books. Hold the button to keep checking. Your tally climbs; the share of the Library seen does not visibly change, and the lenses show why.

**Find.** Write anything up to a page long, and choose whether it sits among noise or on an otherwise blank page. _Find it_ shows the page and its location. Letters and marks the Library lacks are respelled, and you're told how. _Look it up_ asks the Library for the page at that location, to show it's the same page. The same text amid noise lands somewhere new each time, because the noise around it differs; on a blank page it always lands in the same place.

**Reckon.** Pick a search party, or set the dials yourself in powers of ten: searchers, books each checks per second, and years searching. The figures show how long reading every book would take, how many digits the effort removes from that wait, and what share of the Library the party would see in the years given.

**Fall.** _Let go._ Each fall takes three days and adds a death to the tally on the rail. With reduced motion set in your system, the fall happens at once.

**Time.** Your subjective time here, against the age of the universe. _Start the stay over_ clears your tally on this device.
